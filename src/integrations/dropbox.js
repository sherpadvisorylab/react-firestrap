import {fetchWithRetry} from "../libs/fetch.js";
import {sleep} from "../libs/utils.js";
import {AuthButton, getAccessToken, useAccessToken} from "../auth.js";
import type {IButton} from "../components/Buttons.js";
import pathInfo from "../libs/path.js";

export const DROPBPX_URL = "https://www.dropbox.com/home";
export const DROPBOX_AUTH_SERVER = 'www.dropbox.com/oauth2';

const fetchDropboxBlob = async (url, body, headers = {}) => {
    const ACCESS_TOKEN= await getAccessToken(process.env.REACT_APP_DROPBOX_CLIENT_ID);
    if(!ACCESS_TOKEN) {
...
export const dropBoxBasePath = (path, includeHost = false) => {
    return (includeHost ? DROPBPX_URL : "") + (path.startsWith(process.env.REACT_APP_DROPBOX_BASE_PATH) ? path : process.env.REACT_APP_DROPBOX_BASE_PATH + path);
}

export const getDropBoxPaths = async (jobPath, startPath = null, removeExtension = false) => {
    const paths = {};
...
const createFolders = async (paths, basePath = "") => {
    ...
    const response = await fetchDropbox(`https://api.dropboxapi.com/2/${apiPath}`, {
        "autorename": false,
        "force_async": true,
        "paths": paths.map(path => dropBoxBasePath(basePath + path)),
    });
...
const deleteBulk = async (paths, basePath = "") => {
    ...
    const response = fetchDropbox(`https://api.dropboxapi.com/2/${apiPath}`, {
        entries: paths.map(path => ({ path: dropBoxBasePath(basePath + path) })),
    });
...
const addTags = async (paths, basePath = "") => {
    for (const path in paths) {
        for (const tag of paths[path]) {
            await addTag(dropBoxBasePath(basePath + path), tag);
        }
    }
}
...
const copy_ = async (paths) => {
    ...
    for (const path of paths) {
        await startCopy({
            from_path: dropBoxBasePath(path.from),
            to_path: dropBoxBasePath(path.to),
        });
    }
...
export const useDropBoxConnect = (renew = false) => {
    return useAccessToken(process.env.REACT_APP_DROPBOX_CLIENT_ID, renew);
}

export const DropBoxAuthButton = (options: IButton = {}) => {
